document.addEventListener('DOMContentLoaded', () => {
  ...
  let apiKey = window.CONFIG?.OPENROUTER_API_KEY || localStorage.getItem('openrouter_api_key');

  // If we got the API key from CONFIG, save it to localStorage for future use
  if (window.CONFIG?.OPENROUTER_API_KEY && !localStorage.getItem('openrouter_api_key')) {
    localStorage.setItem('openrouter_api_key', window.CONFIG.OPENROUTER_API_KEY);
    console.log("API key saved to localStorage from config");
  }
  ...
  async function handleSearch() {
    ...
    if (!apiKey) {
      showError("Please set your OpenRouter API key in the settings (gear icon in the top left).");
      return;
    }
